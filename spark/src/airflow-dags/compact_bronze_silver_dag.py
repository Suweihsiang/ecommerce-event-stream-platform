...
with DAG(
    dag_id="compact_bronze_silver",
    default_args=default_args,
    start_date=pendulum.datetime(2026, 5, 9, 3, 0, tz=local_tz),
    schedule="0 */6 * * *",  # 每天 UTC 03:00 跑一次
    catchup=False,
    tags=["spark", "bronze", "silver", "maintenance", "compact"],
) as dag:

    compact_bronze_silver = KubernetesPodOperator(
        task_id="compact_bronze_silver",
        name="compact-bronze-silver",
        namespace="spark",
        ...
        arguments=[
            ...
            "local:///opt/spark/apps/compact-bronze-silver.py",
        ],
        get_logs=True,
